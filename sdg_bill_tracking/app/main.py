...
def load_format_tagged_bills(sdg_dict, target_dict):
    ...
    bills['tagged_sdg_text'] = bills.tagged_sdg.apply(lambda x: sdg_dict[x] if pd.notna(x) else None)
    bills['tagged_target_text'] = bills.tagged_target.apply(lambda x: target_dict[x] if pd.notna(x) else None)
    return bills
...
def apply_sdg_filter(bills, selected_sdgs, selected_targets):
    ...
    if selected_sdgs:
        bills_filtered = bills_filtered[bills_filtered.tagged_sdg_text.isin(set(selected_sdgs))]
    if selected_targets:
        bills_filtered = bills_filtered[bills_filtered.tagged_target_text.isin(set(selected_targets))]
